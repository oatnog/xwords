...
class Crossword(object):
    ...
    def randomize_word_list(self):  # also resets words and sorts by length
        ...
        temp_list.sort(key=lambda i: len(i.word),
    ...
    def sort_coordlist(self, coordlist, word):
        ...
        for coord in coordlist:
            ...
            coord[4] = self.check_fit_score(
                col, row, vertical, word)  # checking scores
    ...
    def fit_and_add(self, word):
        ...
        while not fit and count < self.maxloops:
            ...
            if len(self.current_word_list) == 0:  # this is the first word: the seed
                ...
                try:
                    col, row, vertical = coordlist[count][
                        0], coordlist[count][1], coordlist[count][2]
                except IndexError:
    ...
    def c_solution(self):  # return solution grid with coffescript tags
        outStr = "<pre>"
        for r in range(self.rows):
            for c in self.grid[r]:
                outStr += '%s ' % c
    ...
    # return (and order/number wordlist) the grid minus the words adding the
    # numbers
    ...
    def legend(self):  # must order first
        ...
        for word in self.current_word_list:
            outStr += '%d. (%d,%d) %s: %s\n' % (word.number,
                                                word.col, word.row, word.down_across(), word.clue)
        return outStr
